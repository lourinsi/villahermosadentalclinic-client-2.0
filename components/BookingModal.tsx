...
import { Calendar as CalendarIcon, Clock, Award, Loader2, CreditCard, Banknote, Stethoscope, ChevronLeft, AlertCircle, Plus, Check, X, ShieldCheck, RotateCcw } from "lucide-react";
...
export default function BookingModal({ open, onOpenChange, defaultDate, defaultTime, doctorName, defaultPatientId, onBooked, onDeleted, appointmentToEdit, title, bookingMode = "standard", appointmentCreationMode = "standard" }: BookingModalProps) {
  ...
  const continueAfterOverpaymentConfirmation = (shouldAdjustPrice: boolean) => {
    ...
    if (shouldAdjustPrice) {
      ...
      const adjustedNetPrice = Number.isFinite(parsedAdjustedPrice) && parsedAdjustedPrice >= 0
        ? parsedAdjustedPrice
        : Math.max(0, previouslyPaidAmount + amount);
  ...
  const handleConfirmPayment = async () => {
    ...
    if (amount > remainingBalance) {
      beginOverpaymentConfirmation(amount);
      return;
    }
  ...
  return (
    <>
                    ...
                    <div className="p-4 bg-white rounded-lg border border-gray-100 shadow-sm space-y-3">
                      ...
                      <div className="flex items-center justify-between gap-3 pb-3 border-b border-gray-100">
                        ...
                        {isDoctorSelectionLocked ? (
                          ...
                          <Select value={selectedDoctorSelectValue} onValueChange={(newDoctor) => {
      ...
      <SelectPatientModal
        ...
        selectedPatientName={patients.find((p) => p.id === selectedPatient)?.name}
        canCreatePatients={!isPatientReadonly}
        onConfirm={(patient) => {
      ...
      <SelectTreatmentModal
        ...
        treatments={(serviceOptions || []).map((opt: any, idx: number) => ({
          id: idx + 1,
      ...
      <DatePickerModal
        ...
        doctorName={selectedDoctorForSchedule}
        patientId={selectedPatient}
        selectedTime={selectedTime}
        duration={duration}
