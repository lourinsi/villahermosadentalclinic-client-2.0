                      ...
                      <div className="relative space-y-5 pl-7 sm:pl-11">
                        ...
                        {filteredDetailedExpenses.map((expense) => {
                          ...
                          return <article key={expense.id} className="relative"><span className={`absolute -left-[1.55rem] top-9 h-4 w-4 rounded-full border-4 border-white shadow sm:-left-[2.15rem] ${deleted ? "bg-slate-400" : normalizedStatus === "paid" ? "bg-emerald-500" : "bg-violet-600"}`} /><div className={`rounded-2xl border p-4 shadow-sm transition hover:border-violet-200 hover:shadow-md sm:p-5 ${deleted ? "border-red-100 bg-red-50/40" : "border-slate-200 bg-white"}`}>
                            ...
                            <div className="flex flex-col gap-4 sm:flex-row sm:items-center"><div className="grid grid-cols-3 gap-4 sm:min-w-[330px]"><div><p className="text-[10px] font-black uppercase tracking-wider text-slate-400">Price</p>{Number((expense as any).discount) > 0 ? (<div className="mt-1 flex flex-col leading-tight"><span className="text-[10px] font-normal text-slate-400 line-through decoration-rose-400">{formatCurrency(totalPrice)}</span><span className="font-black text-slate-950">{formatCurrency(Math.max(0, totalPrice - Number((expense as any).discount)))}</span></div>) : (<p className="mt-1 font-black text-slate-950">{formatCurrency(totalPrice)}</p>)}</div><div><p className="text-[10px] font-black uppercase tracking-wider text-slate-400">Paid</p><p className="mt-1 font-black text-emerald-600">{formatCurrency(totalPaid)}</p></div><div><p className="text-[10px] font-black uppercase tracking-wider text-slate-400">Balance</p><p className={`mt-1 font-black ${balance > 0 ? "text-amber-600" : "text-emerald-600"}`}>{formatCurrency(balance)}</p></div></div><div className="flex gap-2"><Button variant="outline" size="sm" onClick={() => openExpenseSnapshot(expense)}><Eye className="mr-1 h-3.5 w-3.5" />View</Button>{!deleted ? <Button size="sm" onClick={() => openExpensePaymentModal(expense)} className="bg-violet-600 hover:bg-violet-700"><Plus className="mr-1 h-3.5 w-3.5" />Payment</Button> : null}<DropdownMenu><DropdownMenuTrigger asChild><Button variant="outline" size="icon" className="h-9 w-9" aria-label="More expense actions"><MoreHorizontal className="h-4 w-4" /></Button></DropdownMenuTrigger><DropdownMenuContent align="end">{deleted ? <DropdownMenuItem onClick={() => handleRestoreExpense(expense)}><RotateCcw className="mr-2 h-4 w-4" />Restore expense</DropdownMenuItem> : <><DropdownMenuItem onClick={() => openExpenseModal("edit", expense)}><Edit className="mr-2 h-4 w-4" />Edit expense</DropdownMenuItem>{canDeleteExpenses ? <DropdownMenuItem onClick={() => openExpenseDeleteDialog(expense)} className="text-red-700"><Trash2 className="mr-2 h-4 w-4" />Delete expense</DropdownMenuItem> : null}</>}</DropdownMenuContent></DropdownMenu></div></div></div>
